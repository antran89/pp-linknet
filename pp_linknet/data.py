...
def default_loader(id, root, shape):
    ...
    if mask.shape != shape:
        img, mask = randomCrop(img, mask, shape)
...
def default_loader2(id, root, shape):
    img = cv2.imread(os.path.join(root,'{}_sat.jpg').format(id))
    mask = cv2.imread(os.path.join(root, '{}_mask.png').format(id), cv2.IMREAD_GRAYSCALE)
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, trainlist, root, shape):
        self.ids = trainlist
        self.loader = default_loader2
        self.root = root
        self.shape = shape

    def __getitem__(self, index):
        id = self.ids[index]
        img, mask = self.loader(id, self.root, self.shape)
